...
def track_execution_times(algo_name, goals, width, resolution, origin):
    ...
    for segment_idx, (goal_x, goal_y) in enumerate(goals_world, start=1):
        ...
        rospy.loginfo(
            "[%s] Segment %d: Waiting for robot to reach goal...",
            algo_name,
            segment_idx,
        )
...
def run_algorithm(
    algo_func, algo_name, start, goals, width, height, costmap, resolution, origin
):
    ...
    for segment_idx, goal in enumerate(goals, start=1):
        ...
        grid_visualisation = GridViz(
            costmap, resolution, origin, current_start, goal, width
        )
        ...
        if not path:
            ...
            write_metric(
                algo_name, segment_idx, current_start, goal, 0, computation_time
            )
        else:
            write_metric(
                algo_name, segment_idx, current_start, goal, len(path), computation_time
            )
            final_path.extend(path)
            rospy.loginfo(
                "[%s] Segment %d: Path sent to navigation stack", algo_name, segment_idx
            )
    return final_path
...
def make_plan(req) -> PathPlanningPluginResponse:
    ...
    if algo_param in available_algorithms:
        ...
        rospy.logwarn(
            f"Unknown algorithm '{algo_param}', defaulting to standard",
        )
